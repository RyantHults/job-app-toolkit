...
- Adding a new module requires: new `modules/<id>/` files, registration via `window.jobAppToolkit.registerModule({ id, name, optionsUrl, handleMessage, createContextMenu, quickActions, exportData, importData })` in its background script, and new entries in `manifest.json` (background script + content script). `exportData(api, opts)` / `importData(api, exported)` are the optional export/import hooks (same mechanism as the other registration fields); modules without them fall back to exporting the sync payload minus `active` and importing via `setModuleData` + `setModuleActive`. `core/content.js` and `core/storage.js` are loaded by the popup/options pages too.
